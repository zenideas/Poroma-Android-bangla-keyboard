...
public class CandidateView extends View {
	...
	public CandidateView(Context context, AttributeSet attrs) {
		...
		mMinTouchableWidth = (int) res
				.getDimension(R.dimen.candidate_min_touchable_width);
	...
	private class CandidateStripGestureListener extends
		...
		public void onLongPress(MotionEvent me) {
			if (mSuggestions.size() > 0) {
				if (me.getX() + getScrollX() < mWordWidth[0]
						&& getScrollX() < 10) {
					longPressFirstWord();
				}
			}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < count; i++) {
			...
			// this line is for bangla rendering of the candidate view text
//			suggestion = BanglaRenderer.convert((mSuggestions.get(i)).toString());
			if (suggestion == null)
				continue;
			...
			// Candidate View এ বাংলা ফন্ট ।।
			paint.setTypeface(Typeface.createFromAsset(
					getContext().getAssets(), "fonts/banglaFont.ttf"));
			int wordWidth;
			...
			if (canvas != null) {
				canvas.drawText(suggestion, 0, wordLength, x + wordWidth / 2,
						y, paint);
				paint.setColor(mColorOther);
	...
	private void showPreview(int wordIndex, String altText) {
		...
		if (oldWordIndex != mCurrentWordIndex || altText != null) {
			if (wordIndex == OUT_OF_BOUNDS_WORD_INDEX) {
				...
				CharSequence word = altText != null ? altText : mSuggestions
						.get(wordIndex);
				...
				int wordWidth = (int) (mPaint.measureText(word, 0,
						word.length()) + X_GAP * 2);
				...
				mPopupPreviewX = mWordX[wordIndex]
						- mPreviewText.getPaddingLeft() - getScrollX()
				...
				if (mPreviewPopup.isShowing()) {//
					mPreviewPopup.update(mPopupPreviewX, mPopupPreviewY
							+ offsetInWindow[1], popupWidth, popupHeight);
				} else {
	...
	private void longPressFirstWord() {
		...
		if (mService.addWordToDictionary(word.toString())) {
			showPreview(
					0,
					getContext().getResources().getString(R.string.added_word,
							word));
		}
	}
