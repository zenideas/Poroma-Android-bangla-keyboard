...
/**
 * A view that renders a virtual {@link LatinKeyboard}. It handles rendering of
 * keys and detecting key presses and touch movements.
 * 
 * TODO: References to LatinKeyboard in this class should be replaced with ones
 * to its base class.
 * 
 ...
 */
public class LatinKeyboardBaseView extends View implements
	...
	public interface OnKeyboardActionListener {
		 ...
		 * @param keyCodes
		 *            the codes for all the possible alternative keys with the
		 *            primary code being the first. If the primary key code is a
		 *            single character such as an alphabet or number or symbol,
		 *            the alternatives will include other characters that may be
		 *            on the same key or adjacent keys. These codes are useful
		 *            to correct for accidental presses of a key adjacent to the
		 *            intended key.
		 * @param x
		 *            x-coordinate pixel of touched event. If onKey is not
		 *            called by onTouchEvent, the value should be
		 *            NOT_A_TOUCH_COORDINATE.
		 * @param y
		 *            y-coordinate pixel of touched event. If onKey is not
		 *            called by onTouchEvent, the value should be
		 *            NOT_A_TOUCH_COORDINATE.
		 */
	...
	// This map caches key label text height in pixel as value and key label
	// text size as map key.
	private final HashMap<Integer, Integer> mTextHeightCache = new HashMap<Integer, Integer>();
	// Distance from horizontal center of the key, proportional to key label
	// text height.
	...
	public LatinKeyboardBaseView(Context context, AttributeSet attrs,
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.LatinKeyboardBaseView, defStyle,
				R.style.LatinKeyboardBaseView);
		...
		mDelayBeforePreview = res
				.getInteger(R.integer.config_delay_before_preview);
		mDelayAfterPreview = res
				.getInteger(R.integer.config_delay_after_preview);
		...
		mHasDistinctMultitouch = context.getPackageManager().hasSystemFeature(
				PackageManager.FEATURE_TOUCHSCREEN_MULTITOUCH_DISTINCT);
		mKeyRepeatInterval = res
				.getInteger(R.integer.config_key_repeat_interval);
	}
	...
	public void setKeyboard(Keyboard keyboard) {
		...
		mKeyboardVerticalGap = (int) getResources().getDimension(
				R.dimen.key_bottom_gap);
		for (PointerTracker tracker : mPointerTrackers) {
	...
	private void onBufferDraw() {
		if (mBuffer == null || mKeyboardChanged) {
			if (mBuffer == null
				...
				mBuffer = Bitmap.createBitmap(width, height,
		...
		for (int i = 0; i < keyCount; i++) {
			...
			if (label != null) {
				...
				// custom font add করেছি ।
				paint.setTypeface(Typeface.createFromAsset(getContext()
						.getAssets(), "fonts/banglaFont.ttf"));
				...
				if (labelHeightValue != null) {
					...
					paint.getTextBounds(KEY_LABEL_HEIGHT_REFERENCE_CHAR, 0, 1,
							textBounds);
				...
				final float baseline = centerY + labelHeight
						* KEY_LABEL_VERTICAL_ADJUSTMENT_FACTOR;
				canvas.drawText(label, centerX, baseline, paint);
				// Turn off drop shadow
				paint.setShadowLayer(0, 0, 0, 0);

				// Usually don't draw icon if label is not null, but we draw
				// icon for the number
				// hint and popup hint.
	...
	public void showPreview(int keyIndex, PointerTracker tracker) {
		...
		final boolean hidePreviewOrShowSpaceKeyPreview = (tracker == null)
				|| tracker.isSpaceKey(keyIndex)
				|| tracker.isSpaceKey(oldKeyIndex);
...
//				mPreviewText.setTypeface(Typeface.DEFAULT_BOLD);
				mPreviewText.setTypeface(Typeface.createFromAsset(getContext()
						.getAssets(), "fonts/banglaFont.ttf"));
			} else {
				mPreviewText.setTextSize(TypedValue.COMPLEX_UNIT_PX,
...
//				mPreviewText.setTypeface(mKeyTextStyle);
		...
		int popupWidth = Math.max(
				...
				key.width + mPreviewText.getPaddingLeft()
	...
	private View inflateMiniKeyboardContainer(Key popupKey) {
		...
		miniKeyboard
				.setOnKeyboardActionListener(new OnKeyboardActionListener() {
					public void onKey(int primaryCode, int[] keyCodes, int x,
							int y) {
						mKeyboardActionListener.onKey(primaryCode, keyCodes, x,
								y);
		...
		miniKeyboard.mKeyDetector = new MiniKeyboardKeyDetector(
				mMiniKeyboardSlideAllowance);

		Keyboard keyboard;
		if (popupKey.popupCharacters != null) {
			keyboard = new Keyboard(getContext(), popupKeyboardId,
					popupKey.popupCharacters, -1, getPaddingLeft()
							+ getPaddingRight());
		} else {
			keyboard = new Keyboard(getContext(), popupKeyboardId);
		}
	...
	protected boolean onLongPress(Key popupKey) {
		// TODO if popupKey.popupCharacters has only one letter, send it as key
		// without opening
		// mini keyboard.
		...
		mMiniKeyboard = (LatinKeyboardBaseView) container
				.findViewById(R.id.LatinKeyboardBaseView);
		...
		// On the other hand, "popupKey.width" is width of the pressed key on
		// the main keyboard.
		// We adjust the position of mini popup keyboard with the edge key in
		// it:
		// a) When we have the leftmost key in popup keyboard directly above the
		// pressed key
		// Right edges of both keys should be aligned for consistent default
		// selection
		// b) When we have the rightmost key in popup keyboard directly above
		// the pressed key
		// Left edges of both keys should be aligned for consistent default
		// selection
		final List<Key> miniKeys = mMiniKeyboard.getKeyboard().getKeys();
		final int miniKeyWidth = miniKeys.size() > 0 ? miniKeys.get(0).width
				: 0;

		// HACK: Have the leftmost number in the popup characters right above
		// the key
		boolean isNumberAtLeftmost = hasMultiplePopupChars(popupKey)
				&& isNumberAtLeftmostPopupChar(popupKey);
		int popupX = popupKey.x + mWindowOffset[0];
		popupX += getPaddingLeft();
		if (isNumberAtLeftmost) {
			popupX += popupKey.width - miniKeyWidth; // adjustment for a)
														// described above
		...
		final int x = popupX;
		final int y = mShowPreview && isOneRowKeys(miniKeys) ? mPopupPreviewDisplayedY
				: popupY;
	...
	private boolean isLatinF1Key(Key key) {
		return (mKeyboard instanceof LatinKeyboard)
				&& ((LatinKeyboard) mKeyboard).isF1Key(key);
	}
	...
	/* package */static boolean isNumberAtRightmostPopupChar(Key key) {
		if (key.popupCharacters != null
				...
				&& isAsciiDigit(key.popupCharacters.charAt(key.popupCharacters
						.length() - 1))) {
			return true;
		}
		return false;
	...
	public boolean onTouchEvent(MotionEvent me) {
		...
		// We must disable gesture detector while mini-keyboard is on the
		// screen.
		...
		// TODO: cleanup this code into a multi-touch to single-touch event
		// converter class?
		// Translate mutli-touch event to single-touch events on the device that
		// has no distinct
		// multi-touch panel.
...
//						+ pointerCount + " (old " + oldPointerCount + ")");
	...
	private void onDownEvent(PointerTracker tracker, int x, int y,
			long eventTime) {
