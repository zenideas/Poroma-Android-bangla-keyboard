...
public class LatinIME extends InputMethodService implements
	...
	phoneticBangla phoneticConv = new phoneticBangla();
	...
	public void onUpdateSelection(int oldSelStart, int oldSelEnd,
		...
		if ((((mComposing.length() > 0 && mPredicting) || mVoiceInputHighlighted)
			...
			mPredicting = false;
			if (isPhoneticMode()) {
				mWord = stringTOWordComposer(phoneticConv.phonetic(new StringBuilder(mComposing))
						.toString());	
			}
	...
	boolean isPhoneticMode() {
		return mLanguageSwitcher.getInputLanguage().equals("ph");
	}
				...
				if (inputConnection != null) {
					...
					if (isPhoneticMode()) {
						inputConnection.commitText(
								phoneticConv.phonetic(
										new StringBuilder(mComposing))
										.toString(), 1);
					} else {
						inputConnection.commitText(mComposing, 1);
					}
	...
	private void postUpdateShiftKeyState() {
		...
		mHandler.sendMessageDelayed(
				mHandler.obtainMessage(MSG_UPDATE_SHIFT_STATE), 300);
	}
	...
	private void swapPunctuationAndSpace() {
		...
		if (lastTwo != null && lastTwo.length() == 2
			...
			if (isPhoneticMode()) {
				ic.commitText(
						phoneticConv.phonetic(
								new StringBuilder(lastTwo.charAt(1) + " "))
								.toString(), 1);
			} else {
				ic.commitText(lastTwo.charAt(1) + " ", 1);
			}
	...
	public void onText(CharSequence text) {
		...
		if (isPhoneticMode()) {
			ic.commitText(phoneticConv.phonetic(new StringBuilder(text))
					.toString(), 1);
		} else {
			ic.commitText(text, 1);
		}
		...
		if (mPredicting) {
			...
			if (length > 0) {
				...
				if (isPhoneticMode()) {
					ic.setComposingText(
							phoneticConv
									.phonetic(new StringBuilder(mComposing))
									.toString(), 1);
				} else {
					ic.setComposingText(mComposing, 1);
				}
	...
	public WordComposer stringTOWordComposer(String s) {
		WordComposer wc = new WordComposer();
		for (int i = 0; i < s.length(); i++) {
			wc.add(s.charAt(i), new int[] { s.charAt(i) });
		}
		return wc;
	}
...
//		 && !isCursorTouchingWord()
	...
	public void pickSuggestionManually(int index, CharSequence suggestion) {
		if (mAfterVoiceInput && mShowingVoiceSuggestions)
			mVoiceInput.logNBestChoose(index);
		...
		LatinImeLogger.logOnManualSuggestion(mComposing.toString(),
	...
	private void pickSuggestion(CharSequence suggestion, boolean correcting) {
		...
		if (ic != null) {
			...
			if (isPhoneticMode()) {
				ic.commitText(
						phoneticConv.phonetic(new StringBuilder(suggestion))
								.toString(), 1);
			} else {
				ic.commitText(suggestion, 1);
			}
	...
	public void revertLastWord(boolean deleteChar) {
		...
		if (!mPredicting && length > 0) {
			...
			if (isPhoneticMode()) {
				ic.setComposingText(
						phoneticConv.phonetic(new StringBuilder(mComposing))
								.toString(), 1);
			} else {
				ic.setComposingText(mComposing, 1);
			}
		...
		if (PREF_SELECTED_LANGUAGES.equals(key)) {
			...
			mReCorrectionEnabled = sharedPreferences.getBoolean(
					...
					getResources().getBoolean(
							R.bool.default_recorrection_enabled));
		}
	}
