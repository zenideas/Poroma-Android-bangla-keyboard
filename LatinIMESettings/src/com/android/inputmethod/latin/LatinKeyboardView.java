...
public class LatinKeyboardView extends LatinKeyboardBaseView {
	...
	protected CharSequence adjustCase(CharSequence label) {
		...
		if (keyboard.isShifted() && keyboard instanceof LatinKeyboard
	...
	/**
	 * This function checks to see if we need to handle any sudden jumps in the
	 * pointer location that could be due to a multi-touch being treated as a
	 * move by the firmware or hardware. Once a sudden jump is detected, all
	 * subsequent move events are discarded until an UP is received.
	 * <P>
	 * When a sudden jump is detected, an UP event is simulated at the last
	 * position and when the sudden moves subside, a DOWN event is simulated for
	 * the second key.
	 * 
	 * @param me
	 *            the motion event
	 * @return true if the event was consumed, so that it doesn't continue to be
	 *         handled by KeyboardView.
	 */
	private boolean handleSuddenJump(MotionEvent me) {
		...
		case MotionEvent.ACTION_MOVE:
			// Is this a big jump?
			final int distanceSquare = (mLastX - x) * (mLastX - x)
					+ (mLastY - y) * (mLastY - y);
			// Check the distance and also if the move is not entirely within
			// the bottom row
			// If it's only in the bottom row, it might be an intentional slide
			// gesture
			// for language switching
			if (distanceSquare > mJumpThresholdSquare
					&& (mLastY < mLastRowY || y < mLastRowY)) {
				// If we're not yet dropping events, start dropping and send an
				// UP event
				if (!mDroppingEvents) {
					...
					MotionEvent translated = MotionEvent.obtain(
							...
							MotionEvent.ACTION_UP, mLastX, mLastY,
							me.getMetaState());
				...
				// If moves are small and we're already dropping events,
				// continue dropping
				result = true;
			}
			break;
		case MotionEvent.ACTION_UP:
			if (mDroppingEvents) {
				// Send a down event first, as we dropped a bunch of sudden
				// jumps and assume that
				// the user is releasing the touch on the second key.
				MotionEvent translated = MotionEvent.obtain(me.getEventTime(),
						me.getEventTime(), MotionEvent.ACTION_DOWN, x, y,
						me.getMetaState());
				super.onTouchEvent(translated);
				translated.recycle();
				mDroppingEvents = false;
	...
	public boolean onTouchEvent(MotionEvent me) {
		...
		// If there was a sudden jump, return without processing the actual
		// motion event.
		...
		if (me.getAction() == MotionEvent.ACTION_UP) {
			...
			if (languageDirection != 0) {
				getOnKeyboardActionListener().onKey(
						languageDirection == 1 ? KEYCODE_NEXT_LANGUAGE
								: KEYCODE_PREV_LANGUAGE, null, mLastX, mLastY);
				...
				mHandler2 = new Handler() {
					...
					public void handleMessage(Message msg) {
						...
						case MSG_TOUCH_DOWN:
							...
							me.recycle();
							sendEmptyMessageDelayed(MSG_TOUCH_UP, 500); // Deliver
																		// up in
																		// 500ms
																		// if
																		// nothing
						...
						case MSG_TOUCH_UP:
							...
							me2.recycle();
							sendEmptyMessageDelayed(MSG_TOUCH_DOWN, 500); // Deliver
																			// up
																			// in
																			// 500ms
																			// if
